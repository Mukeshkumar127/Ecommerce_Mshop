import React from 'react'

const HomeSectionCard = ({product}) => {
  return (
    <div className='cursor-pointer flex flex-col items-center bg-white rounded-lg shadow-lg overflow-hidden w-[15rem] mx-3 border'>

      <div className='h-[13rem] w-[10rem]'>
        <img className='object-cover object-top w-full h-full' src={product.imageUrl} alt="" />

      </div>

      <div className='p-4 w-full text-center'>
        <h3 className='text-lg font-medium text-gray-900 line-clamp-2 h-[2.7rem] overflow-hidden'>{product.brand}</h3>
        <p className='mt-2 text-sm text-gray-500 line-clamp-2 h-[2.3rem] overflow-hidden'>{product.title}</p>
      </div>

    </div>
  )
}

export default HomeSectionCard